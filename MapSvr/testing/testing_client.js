...
loadProtobuf().then(root => {
    ...
    const ProtoCSReqLogin = root.lookupType("ProtoCSReqLogin")
    const PROTO_CMD_CS_REQ_LOGIN = ProtoCmd.values['PROTO_CMD_CS_REQ_LOGIN']
    const ProtoCSResLogin = root.lookupType("ProtoCSResLogin")
    const PROTO_CMD_CS_RES_LOGIN = ProtoCmd.values['PROTO_CMD_CS_RES_LOGIN']
    ...
    const createCSReqLoginPackage = () => {
        const csReqLogin = ProtoCSReqLogin.create({
            userId: "1",
            password: "alice123"
        });
        const reqPackage = ProtoPackage.create({
